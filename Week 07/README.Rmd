...
audiobooks. There's a book, which you should read. If it isn't good
enough to read, why would you pay someone to read it to you? 

It is worth pointing out there are two playlists with the same
name. In this case, "Audiobooks". But note how they have different
PlaylistId numbers. This _is_ important:
...
A primary key _MUST_ be unique. It cannot have a duplicate (dupe). The
playlist names can dupe (although doing so will cause some confusion
among the humans) but for the database, `PlaylistId` "4" and "6" _are_
unique.
...
you can get the right answer to the very important question waiting
for you there.
...
the `Tracks` table. A reference table differs from a table like
